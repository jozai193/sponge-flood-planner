...
class Solver:
    def __init__(self, surface: Surface, config: SolverConfig | None = None,
                 ...
                 coastal: CoastalBoundary | None = None):
    ...
    def stable_dt(self) -> float:
        ...
        reservoir_speed=0.
        if self.coastal:
            peak=max(k[1] for k in self.coastal.levels)
            lowest=min(self.surface.z.flat[i] for i in self.coastal.cells)
            reservoir_speed=np.sqrt(G*max(0.,peak-lowest))*(1/self.surface.dx+1/self.surface.dy)
    ...
    def _faces(self, state: Array, axis: int, boundary_time: float | None = None) -> tuple[Array, Array, Array, Array]:
        ...
        if self.coastal and axis == (1 if self.coastal.edge in ('west','east') else 0):
            ...
            for i in self.coastal.cells:
                row,col=divmod(i,nx)
                negative=self.coastal.edge in ('west','south')
    ...
    def step(self, dt: float, rain_m_s: float = 0.) -> float:
        ...
        if self.coastal:
            future=[t for t,_ in self.coastal.levels if t>self.time+1e-7]
    ...
    def checkpoint(self) -> dict:
        return {"version": "0.1.0", "u": self.u.copy(), "soil": self.soil.copy(),
    ...
    def restore(self, checkpoint: dict) -> None:
        if checkpoint.get("version") != "0.1.0" or checkpoint["u"].shape != self.u.shape:
