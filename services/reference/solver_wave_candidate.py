"""Isolated wave-boundary candidate; production and earlier candidate stay frozen."""
import hashlib
import json
...
class Solver(SegmentSolver):
    def __init__(self, *args, **kwargs):
        ...
        extra=[(name,b.background_stage_m,b.signal_kind) for name,b in self.coastal_segments
               if isinstance(b,CharacteristicBoundary)]
        self.boundary_fingerprint=hashlib.sha256(json.dumps([self.boundary_fingerprint,extra]).encode()).hexdigest()
        ...
        for _, boundary in self.coastal_segments:
            if axis == (1 if boundary.edge in ('west','east') else 0):
                ...
                for i in boundary.cells:
                    ...
                    h=max(0.,level-bed)
                    if isinstance(boundary, CharacteristicBoundary):
                        ghost=wave_ghost(fluid,bed,level,boundary.background_stage_m,boundary.edge,self.config.dry_depth_m)
                    else:
