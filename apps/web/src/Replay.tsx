...
import {formatMoneyMinor,normaliseCurrency} from '../../../packages/domain/currency';
...
type ComparisonResult={baselineScore:number;plannedScore:number;designs:PhysicalDesign[];evidence?:{currency?:string;budgetMinor?:number;searchResult?:{status:string;evaluations:number;elapsedMs?:number;terminationReason?:string}}};
...
export default function Replay({renderQuality='balanced',waterHeightScale=1,baseline,planned,input,plannedInput,extent,result,provenance,context,satelliteImage,imageryDescriptor,onClose}:{renderQuality?:string;waterHeightScale?:number;baseline:Frame[];planned:Frame[];input:GPUInput;plannedInput:GPUInput;extent:number;result:ComparisonResult;provenance:{buildings:CityBuilding[];currency?:string;grid?:unknown};context:CityContext|null;satelliteImage:HTMLCanvasElement|null;imageryDescriptor:ImageryDescriptor|null;onClose:()=>void}){
 ...
 const currency=normaliseCurrency(result.evidence?.currency??provenance.currency??result.designs.find(d=>d.costBreakdown)?.costBreakdown?.currency);
 const valuationCurrencies=[...new Set(provenance.buildings.filter(b=>b.structure_value_minor!==null&&b.structure_value_minor!==undefined).map(b=>b.valuation_currency).filter((value):value is string=>!!value))];
 const lossCurrency=normaliseCurrency(valuationCurrencies.length===1?valuationCurrencies[0]:currency);
 ...
 async function download(kind:'evidence'|'html'|'csv'|'geojson'|'manifest'){
  setExportError('');setExporting(true);try{const report=await buildReport({input,plannedInput,baseline,planned,result,provenance});
   const spatial=kind==='geojson'?(await import('../../../packages/metrics/geojson')).selectedGeoJSON(input,report.scenario.selectedDesigns,(provenance as any).grid,(report.scenario.evidence as any).budgetMinor,currency):null;
 ...
 return <section className="replay" role="dialog" aria-label="Storm comparison">
  ...
  <p>Peak excess-depth score outside candidate areas: {result.baselineScore.toFixed(1)} → {result.plannedScore.toFixed(1)} m³ {reduction!==null&&`(${reduction.toFixed(1)}% reduction)`}. Assumed installation cost: {formatMoneyMinor(result.designs.reduce((s,d)=>s+d.costMinor,0),currency)} {currency}.</p>
