...
import {formatMoneyMinor,normaliseCurrency} from '../domain/currency';
...
export async function buildReport(request:ReportRequest){
 ...
 const declaredCurrency=e.currency??provenance.currency??designs.find((d:any)=>d.costBreakdown)?.costBreakdown?.currency??'USD';
 if(typeof declaredCurrency!=='string'||!/^[A-Z]{3}$/.test(declaredCurrency))throw new Error('Invalid report currency');
 const currency=normaliseCurrency(declaredCurrency);
 if(designs.some((d:any)=>d.costBreakdown&&d.costBreakdown.currency!==currency))throw new Error('Design cost currency does not match report currency');
 ...
 if(candidates.some((d:any)=>d.costBreakdown&&d.costBreakdown.currency!==currency))throw new Error('Candidate cost currency does not match report currency');
