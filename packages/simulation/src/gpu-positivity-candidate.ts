// EXPERIMENTAL ONLY. Production/historical harness imports remain on gpu.ts.
import {correctDepthRoundoff,validateRoundoffBudget} from './positivity-roundoff';
import type {GPUCheckpoint as BaselineCheckpoint} from './gpu';
...
export interface GPUCheckpoint {version:2;roundoffVolume:number;inputHash:string;state:Float32Array;history:Float32Array;time:number;steps:number;retries:number;initialVolume:number;rainVolume:number;inflowVolume:number;coastalOutflowVolume?:number}
...
float bad=(any(isnan(s))||any(isinf(s))||s.x<-.0000000001||s.w<0.)?1.:0.;result=vec4(speed,bad,s.x<0.?1.:0.,0.);}`;
...
export class GPUSolver {
  ...
  private roundoffVolume=0;private current=0;private historyCurrent=0;time=0;steps=0;private rainArea=0;private initialVolume=0;private rainVolume=0;private outflowVolume=0;
  ...
  stableDt(correctRoundoff=false){this.draw(this.speedP,[this.speedT],this.base());let previous=this.speedT;
    ...
    const v=this.read(previous);if(v[1]>.5)throw new Error('NUMERICAL_INVALID: non-finite or negative GPU state');if(correctRoundoff&&v[2]>.5){
      const state=this.read(this.states[this.current]);const correction=correctDepthRoundoff(state,this.input.dx*this.input.dy);
      validateRoundoffBudget(this.roundoffVolume+correction.volumeM3,this.initialVolume+this.rainVolume+this.inflowVolume);
      const gl=this.gl;gl.bindTexture(gl.TEXTURE_2D,this.states[this.current].handle);gl.texSubImage2D(gl.TEXTURE_2D,0,0,0,this.input.nx,this.input.ny,gl.RGBA,gl.FLOAT,state);
      this.roundoffVolume+=correction.volumeM3;
    }
    return Math.min(this.input.maxStepS??1,.4/Math.max(v[0]+this.coastalWaveRate,1e-12));}
  ...
  step(requestedDt:number,rain:number){
    ...
    const saved=this.current,savedHistory=this.historyCurrent,savedRoundoff=this.roundoffVolume;
    ...
    for(let attempt=0;attempt<12;attempt++){
      try{
        ...
        const secondExchange=this.stage(this.states[stage],this.states[end],this.states[start],dt,1);this.current=end;this.stableDt(true);this.sources(dt/2,rain);this.stableDt(true);
        ...
        this.roundoffVolume=savedRoundoff;this.current=saved;this.historyCurrent=savedHistory;this.draw(this.copyP,[this.states[saved]],{inputTex:this.backupState});this.draw(this.copyP,[this.histories[savedHistory]],{inputTex:this.backupHistory});
  ...
  async restore(checkpoint:GPUCheckpoint){
    ...
    if(checkpoint.version!==2||checkpoint.inputHash!==await inputIdentity(this.input))throw new Error('Checkpoint input or version mismatch');
    for(const key of ['time','steps','retries','initialVolume','rainVolume','inflowVolume','roundoffVolume'] as const)if(!Number.isFinite(checkpoint[key])||checkpoint[key]<0)throw new Error('Invalid checkpoint counters');
    ...
    for(let i=0;i<this.input.z.length;i++){
      ...
      if(s[j]<0||s[j+3]<0||s[j+3]>this.input.capacity[i]+1e-6||h[j]<s[j]-1e-6||h.subarray(j,j+4).some(v=>v<0))throw new Error('Invalid checkpoint physical state');
    ...
    validateRoundoffBudget(checkpoint.roundoffVolume,supplied);
    ...
    this.roundoffVolume=checkpoint.roundoffVolume;this.time=checkpoint.time;this.steps=checkpoint.steps;this.retries=checkpoint.retries;this.initialVolume=checkpoint.initialVolume;this.rainVolume=checkpoint.rainVolume;this.inflowVolume=checkpoint.inflowVolume;this.coastalOutflowVolume=checkpoint.coastalOutflowVolume??0;
  }
  /** Explicit migration of a baseline checkpoint; input object and arrays remain immutable.
   * The legacy validator runs first. Correction occurs only in the candidate copy.
   */
  async importBaselineCheckpoint(checkpoint:BaselineCheckpoint){
    const {GPUSolver:BaselineSolver}=await import('./gpu');
    const validator=new BaselineSolver(this.input);
    try{await validator.restore(checkpoint);}finally{validator.dispose();}
    const state=checkpoint.state.slice();const correction=correctDepthRoundoff(state,this.input.dx*this.input.dy);
    await this.restore({...checkpoint,version:2,state,roundoffVolume:correction.volumeM3});
    return correction;
  }
  frame(){const state=this.read(this.states[this.current]),history=this.read(this.histories[this.historyCurrent]);const depth=new Float32Array(this.input.nx*this.input.ny),maxDepth=depth.slice(),subsurfaceDepth=depth.slice(),velocityX=depth.slice(),velocityY=depth.slice();let stored=0,subsurface=0,deep=0,outflow=0;
    for(let i=0;i<depth.length;i++){depth[i]=state[i*4];subsurfaceDepth[i]=state[i*4+3];if(depth[i]>1e-5){velocityX[i]=state[i*4+1]/depth[i];velocityY[i]=state[i*4+2]/depth[i];}maxDepth[i]=history[i*4];if(!Number.isFinite(depth[i])||depth[i]<0)throw new Error('NUMERICAL_INVALID: GPU depth');stored+=state[i*4]+state[i*4+3];subsurface+=state[i*4+3];deep+=history[i*4+2];outflow+=history[i*4+3];}
    ...
    return {time_s:this.time,depth,maxDepth,subsurfaceDepth,velocityX,velocityY,steps:this.steps,ledger:{roundoff_correction_m3:this.roundoffVolume,roundoff_adjusted_residual_m3:residual+this.roundoffVolume,initial_m3:this.initialVolume,rain_m3:this.rainVolume,inflow_m3:this.inflowVolume,stored_m3:stored,surface_m3:stored-subsurface*this.input.dx*this.input.dy,subsurface_m3:subsurface*this.input.dx*this.input.dy,deep_percolation_m3:deep,outflow_m3:this.outflowVolume,residual_m3:residual,relative_residual:Math.abs(residual)/Math.max(this.initialVolume+this.rainVolume+this.inflowVolume,1)}};
  }
  info(){const ext=this.gl.getExtension('WEBGL_debug_renderer_info');return {renderer:ext?this.gl.getParameter(ext.UNMASKED_RENDERER_WEBGL):this.gl.getParameter(this.gl.RENDERER),engine:'webgl2-hll-positivity-candidate-v1',spatialOrder:this.input.spatialOrder??1,allocatedBytes:this.textures.reduce((a,t)=>a+t.w*t.h*16,0),readback:'synchronous bounded snapshots'};}
