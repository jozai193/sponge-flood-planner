...
self.onmessage=async(event:MessageEvent)=>{
  ...
  const {input:rawInput,designs,budgetMinor,storm,stormEnsemble,runId,mode}=event.data as {input:GPUInput;designs:PhysicalDesign[];budgetMinor:number;storm:StormRun;stormEnsemble?:StormRun[];runId:string;mode:'PLAN'|'COMPARE'};
